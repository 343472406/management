...
<span id="toolbar" class="l">
	<shiro:hasPermission name="register:batchdel">
		<a href="javascript:;" onclick="register_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="register:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="register_add()"><i class="Hui-iconfont">&#xe600;</i> 
			添加挂号费
		</a>
	</shiro:hasPermission>
</span>
...
<!-- bootstrap -->
<!-- 如果引用bootstrap,会导致引用两次，“每页显示数据量”下拉框触发两次，效果：打不开 -->
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/register/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			/*
				...
				title:表头要显示的名
			*/
			{field: 'registerId',title: '编号'}, 
			{field: 'registerName',title: '挂号费名'}, 
			{field: 'registerTypeName',title: '类型',},
			{field: 'money',title: '挂号金额',},
			{field: 'totalMoney',title: '总费用（挂号金额+子费用）'},
			//操作列的设置（删除，修改）
			/*
		...
		onExpandRow: function (index, row, $detail) {			
			...
			$(cur_table).bootstrapTable({
		            ...
		            columns: [{
		                ...
		                field: 'registerSubTypeName',
		                title: '子费用类型名'
		            },{
		                field: 'registerSubMoney',
		                title: '子费用价格'
		            },]
	        });
	    },
...
/*
	...
	row:	整行数据
	index:	行号
	field:	字段名称：registerId
	
	返回一个字符串，html自动解析渲染效果
*/
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='register:delete'>";
		html += "<span onclick='register_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='register:update'>";
		html += "<span onclick='register_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
...
function register_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加挂号费",
		content: '../system/register/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function register_edit(registerId) {
	layer.open({
		...
		maxmin: true,
		title: "修改挂号费",
		content: '../system/register/editPage.do?registerId='+registerId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
/*挂号费-批量删除*/
function register_batch_del(id) {
	/* 获取被选中的数据 */
	...
	var idsStr = ids.toString();/* 会自动变成1,2,3,4的格式 */
		...
		$.ajax({
			type : 'POST',
			url : '../system/register/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
