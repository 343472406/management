...
<span id="toolbar" class="l">
	<shiro:hasPermission name="user:batchdel">
		<a href="javascript:;" onclick="job_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="user:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="job_add('添加职位','job-role-add.html','800')"><i class="Hui-iconfont">&#xe600;</i> 
			添加职位
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/job/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			*/
			...
			{field: 'jobName',title: '职位名'}, 
			{field: 'jobDesc',title: '描述',},
			{field: 'jobPermissions',title: '权限'},
			//操作列的设置（删除，修改）
			/*
...
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='job:delete'>";
		html += "<span onclick='job_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='job:update'>";
		html += "<span onclick='job_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
...
function job_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加职位",
		content: '../system/job/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function job_edit(jobId) {
	layer.open({
		...
		maxmin: true,
		title: "修改职位",
		content: '../system/job/editPage.do?jobId='+jobId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
