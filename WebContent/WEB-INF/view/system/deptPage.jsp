...
<span id="toolbar" class="l">
	<shiro:hasPermission name="dept:batchdel">
		<a href="javascript:;" onclick="dept_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="dept:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="dept_add('添加科室','dept-role-add.html','800')"><i class="Hui-iconfont">&#xe600;</i> 
			添加科室
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/dept/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			/*
				...
				title:表头要显示的名
			*/
			{field: 'deptId',title: '编号'}, 
			{field: 'deptName',title: '科室名'}, 
			{field: 'deptDesc',title: '科室描述',},
			{field: 'isRegisterDept',title: '是否是接收病人的科室',formatter:isRegisterDeptFormatter,},
			//操作列的设置（删除，修改）
			/*
...
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='dept:delete'>";
		html += "<span onclick='dept_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='dept:update'>";
		html += "<span onclick='dept_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
	
	return html;
}
function isRegisterDeptFormatter(value, row, index, field) {
	var html = "";
	if(value == "1") {
		html = "是";
	} else if(value == "0") {
...
function dept_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加科室",
		content: '../system/dept/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function dept_edit(deptId) {
	layer.open({
		...
		maxmin: true,
		title: "修改科室",
		content: '../system/dept/editPage.do?deptId='+deptId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
function dept_batch_del(id) {
	...
	var idsStr = ids.toString();/* 会自动变成1,2,3,4的格式 */
		...
		$.ajax({
			type : 'POST',
			url : '../system/dept/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
