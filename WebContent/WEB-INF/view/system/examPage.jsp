...
<span id="toolbar" class="l">
	<shiro:hasPermission name="user:batchdel">
		<a href="javascript:;" onclick="exam_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="user:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="exam_add('添加检查项目费','exam-role-add.html','800')"><i class="Hui-iconfont">&#xe600;</i> 
			添加检查项目费
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/exam/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			/*
				...
				title:表头要显示的名
			*/
			{field: 'examId',title: '编号'}, 
			{field: 'examName',title: '检查项目费名'}, 
			{field: 'examTypeName',title: '检查项目费类型',},
			{field: 'price',title: '检查项目费价格(元)'},
			{field: 'cost',title: '检查项目费成本(元)'},
			//操作列的设置（删除，修改）
			/*
...
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='exam:delete'>";
		html += "<span onclick='exam_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='exam:update'>";
		html += "<span onclick='exam_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
...
function exam_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加检查项目费",
		content: '../system/exam/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function exam_edit(examId) {
	layer.open({
		...
		maxmin: true,
		title: "修改检查项目费",
		content: '../system/exam/editPage.do?examId='+examId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
function exam_batch_del(id) {
	...
	var idsStr = ids.toString();/* 会自动变成1,2,3,4的格式 */
		...
		$.ajax({
			type : 'POST',
			url : '../system/exam/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
