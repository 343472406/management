...
<span id="toolbar" class="l">
	<shiro:hasPermission name="permission:batchdel">
		<a href="javascript:;" onclick="permission_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="permission:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="permission_add()"><i class="Hui-iconfont">&#xe600;</i> 
			添加权限
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/permission/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			*/
			...
			{field: 'permissionType',title: '类型',formatter:permissionTypeFormatter,},
...
function operationFormatter(value, row, index, field) {
	...
	html += "<shiro:hasPermission name='permission:delete'>";
		html += "<span onclick='permission_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='permission:update'>";
		html += "<span onclick='permission_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
	
	return html;
}
function permissionTypeFormatter(value, row, index, field) {
	var html = "";
	if(value == "menu") {
		html = "菜单权限";
...
function permission_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加权限",
		content: '../system/permission/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function permission_edit(permissionId) {
	layer.open({
		...
		maxmin: true,
		title: "修改权限",
		content: '../system/permission/editPage.do?permissionId='+permissionId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
/*权限-批量删除*/
function permission_batch_del(id) {
	/* 获取被选中的数据 */
	...
	for(var i=0; i< selects.length; i++) {
		ids.push(selects[i].permissionId);
	}
	var idsStr = ids.toString();/* 会自动变成1,2,3,4的格式 */
		...
		$.ajax({
			type : 'POST',
			url : '../system/permission/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
