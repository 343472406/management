...
<span id="toolbar" class="l">
	<shiro:hasPermission name="user:batchdel">
		<a href="javascript:;" onclick="user_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="user:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="user_add()"><i class="Hui-iconfont">&#xe600;</i> 
			添加员工
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="user:update">
		<a class="btn btn-success radius" href="javascript:;" onclick="user_userBan()"><i class="glyphicon glyphicon-eye-open"></i> 
			已禁用员工列表
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/user/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
...
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='user:update'>";
		/* 管理员可以禁用员工 */
		html += "<button onclick='user_switchStatus("+value+",\"禁用\");' style='cursor:pointer' class='btn btn-danger radius'>禁用</button>&nbsp;&nbsp;";
		/* 管理员可以重置密码 */
		html += "<button onclick='user_resetPassword("+value+");' style='cursor:pointer' class='btn btn-danger radius'>重置密码</button>&nbsp;&nbsp;";
		html += "<span onclick='user_edit("+value+");' style='color:red;cursor:pointer;font-size: 16px;' class='glyphicon glyphicon-pencil'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='user:delete'>";
		html += "<span onclick='user_del(this,"+value+");' style='color:red;cursor:pointer;font-size: 16px;' class='glyphicon glyphicon-trash'></span>";
	html += "</shiro:hasPermission>";
...
function user_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加员工",
		content: '../system/user/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function user_edit(userId) {
	layer.open({
		...
		maxmin: true,
		title: "修改员工",
		content: '../system/user/editPage.do?userId='+userId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
function user_batch_del(id) {
	...
	layer.confirm('确认要删除吗？', function(index) {
		$.ajax({
			type : 'POST',
			url : '../system/user/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
...
/* 员工-重置密码 */
function user_resetPassword(userId) {
...
function user_userBan() {
	layer.open({
		...
		maxmin: true,
		title: "已禁止员工列表",
		content: '../system/user/userBanPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
