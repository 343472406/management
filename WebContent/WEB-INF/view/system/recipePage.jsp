...
<span id="toolbar" class="l">
	<shiro:hasPermission name="user:batchdel">
		<a href="javascript:;" onclick="recipe_batch_del()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i>批量删除
		</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="user:insert">
		<a class="btn btn-primary radius" href="javascript:;" onclick="recipe_add('添加处方附加费','recipe-role-add.html','800')"><i class="Hui-iconfont">&#xe600;</i> 
			添加处方附加费
		</a>
	</shiro:hasPermission>
</span>
...
$(function() {
	...
	$('#dataTable').bootstrapTable({
		url: '../system/recipe/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			/*
				...
				title:表头要显示的名
			*/
			{field: 'recipeId',title: '编号'}, 
			{field: 'recipeName',title: '处方附加费名'}, 
			{field: 'recipeTypeName',title: '处方附加费类型',},
			{field: 'recipeMoney',title: '处方附加费价格(元)'},
			//操作列的设置（删除，修改）
			/*
...
function operationFormatter(value, row, index, field) {
	var html = "";
	html += "<shiro:hasPermission name='recipe:delete'>";
		html += "<span onclick='recipe_del(this,"+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;";
	html += "</shiro:hasPermission>";
	html += "<shiro:hasPermission name='recipe:update'>";
		html += "<span onclick='recipe_edit("+value+");' style='color:red;cursor:pointer' class='glyphicon glyphicon-pencil'></span>";
	html += "</shiro:hasPermission>";
...
function recipe_add() {
	layer.open({
		...
		maxmin: true,
		title: "添加处方附加费",
		content: '../system/recipe/editPage.do',
		/* 遮罩层 */
		shade :0.3,
	});
}
...
function recipe_edit(recipeId) {
	layer.open({
		...
		maxmin: true,
		title: "修改处方附加费",
		content: '../system/recipe/editPage.do?recipeId='+recipeId,
		/* 遮罩层 */
		shade :0.3,
	});    
}
...
/*处方附加费-删除*/
function recipe_del(obj, id) {
		layer.confirm('确认要删除吗？', function(index) {
		$.ajax({
			type : 'POST',
			url : '../system/recipe/delete.do',
			dataType : 'json',
			data: {
				recipeId:id
			},
...
function recipe_batch_del(id) {
	...
	var idsStr = ids.toString();/* 会自动变成1,2,3,4的格式 */
		...
		$.ajax({
			type : 'POST',
			url : '../system/recipe/batchDelete.do',
			dataType : 'json',
			data: {
				ids: idsStr,
			},
